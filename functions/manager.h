...
bool view_all_loans(int connFD);
...
bool manager_operation_handler(int connFD)
{
    ...
    {   printf("Manager Logged in\n");
        ...
        key_t semKey = ftok(EMPLOYEE_FILE, loggedInManager.id); // Generate a key based on the account number hence, different customers will have different semaphores
...
bool deactivate_activate_account(int connFD)
{
    ...
    writeBytes = write(connFD, "Enter the Account Number (e.g. 0, 1, 2) to activate/deactivate", strlen("Enter the Account Number (e.g. 0, 1, 2) to activate/deactivate"));
    ...
    {
        write(connFD, "Please enter a valid integer account number!^", strlen("Please enter a valid integer account number!^"));
    ...
    switch(choice){
        ...
        if (account.balance > 0)
            sprintf(writeBuffer, "Account deactivated. Note: balance of %ld remains.\nRedirecting you to the main menu ...^", account.balance);
        else
            strcpy(writeBuffer, "Account deactivated successfully.\nRedirecting you to the main menu ...^");
...
bool assign_loan(int connFD)
{
    ...
    writeBytes = write(connFD, "Enter the loan ID (e.g. 0, 1, 2) you want to assign", strlen("Enter the loan ID (e.g. 0, 1, 2) you want to assign"));
    bzero(readBuffer, sizeof(readBuffer));
    readBytes = read(connFD, readBuffer, sizeof(readBuffer));

    if (readBuffer[0] < '0' || readBuffer[0] > '9')
    {
        write(connFD, "Please enter a valid integer loan ID!^", strlen("Please enter a valid integer loan ID!^"));
        read(connFD, readBuffer, sizeof(readBuffer));
        return false;
    }
    ...
    writeBytes = write(connFD, "Enter the Employee ID (e.g. 0, 1, 2) to assign", strlen("Enter the Employee ID (e.g. 0, 1, 2) to assign"));
    bzero(readBuffer, sizeof(readBuffer));
    readBytes = read(connFD, readBuffer, sizeof(readBuffer));

    if (readBuffer[0] < '0' || readBuffer[0] > '9')
    {
        write(connFD, "Please enter a valid integer employee ID!^", strlen("Please enter a valid integer employee ID!^"));
        read(connFD, readBuffer, sizeof(readBuffer));
        return false;
    }
...
{
    ssize_t readBytes, writeBytes;
    char readBuffer[1000], writeBuffer[1000], tempBuffer[500];
    ...
    if (loanFileDescriptor == -1)
    {
        write(connFD, "No loans exist in the system!^", strlen("No loans exist in the system!^"));
        read(connFD, readBuffer, sizeof(readBuffer));
        return false;
    }

    off_t fileSize = lseek(loanFileDescriptor, 0, SEEK_END);
    int totalloans = fileSize / sizeof(struct loan);
    ...
    {
        lseek(loanFileDescriptor, i * sizeof(struct loan), SEEK_SET);
        readBytes = read(loanFileDescriptor, &loan, sizeof(struct loan));
        if (readBytes <= 0)
            continue;
        ...
        if (loan.EmployeeID != -1)
            sprintf(tempBuffer, "loan ID: %d | Customer ID: %d | Amount: %ld | Status: %s | Employee ID: %d\n^",
                loan.loanID, loan.customerID, loan.amount,
                (loan.approved == 0 ? "Pending" : (loan.approved == 1 ? "Approved" : "Rejected")),
                loan.EmployeeID);
        else
            sprintf(tempBuffer, "loan ID: %d | Customer ID: %d | Amount: %ld | Status: %s | Unassigned\n^",
                loan.loanID, loan.customerID, loan.amount,
                (loan.approved == 0 ? "Pending" : (loan.approved == 1 ? "Approved" : "Rejected")));

        write(connFD, tempBuffer, strlen(tempBuffer));
        read(connFD, readBuffer, sizeof(readBuffer));
    }
