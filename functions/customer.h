...
bool transfer_funds(int connFD)
{
    ...
    writeBytes = write(connFD, "Enter the Account Number (e.g. 0, 1, 2) of the Receiver", strlen("Enter the Account Number (e.g. 0, 1, 2) of the Receiver"));
    bzero(readBuffer, sizeof(readBuffer));
    readBytes = read(connFD, readBuffer, sizeof(readBuffer));
    ...
    {
        write(connFD, "You cannot transfer funds to your own account!^", strlen("You cannot transfer funds to your own account!^"));
        read(connFD, readBuffer, sizeof(readBuffer));
        return false;
    }
    ...
    if(get_account_details(connFD, &receiverAccount) && get_account_details(connFD, &senderAccount))
    {
        if (receiverAccount.active)
        {
            if (withdrawAmount != 0 && senderAccount.balance - withdrawAmount >= 0)
            {
                ...
                write(connFD, "The amount is successfully withdrawn from your account and credited into receiver's account!^", strlen("The amount is successfully withdrawn from your account and credited into receiver's account!^"));
