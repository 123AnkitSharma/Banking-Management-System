...
bool login_handler(bool isAdmin, int connFD, struct Customer *ptrToCustomerID)
{
    ...
    if (userFound)
    {
        ...
        else
        {
            ...
            {
                struct Account custAccount;
                custAccount.accountNumber = customer.account;
                int accFD = open(ACCOUNT_FILE, O_RDONLY);
                if (accFD != -1)
                {
                    off_t accOff = lseek(accFD, customer.account * sizeof(struct Account), SEEK_SET);
                    if (accOff >= 0)
                    {
                        struct flock accLock = {F_RDLCK, SEEK_SET, accOff, sizeof(struct Account), getpid()};
                        fcntl(accFD, F_SETLKW, &accLock);
                        read(accFD, &custAccount, sizeof(struct Account));
                        accLock.l_type = F_UNLCK;
                        fcntl(accFD, F_SETLK, &accLock);
                    }
                    close(accFD);
                    if (!custAccount.active)
                    {
                        write(connFD, "Your account has been deactivated! Contact the bank manager.$", strlen("Your account has been deactivated! Contact the bank manager.$"));
                        return false;
                    }
                }
...
bool get_employee_details(int connFD, struct Employee *employeeAccount)
{
    ...
    {
        writeBytes = write(connFD, "Enter the Employee ID (e.g. 0, 1, 2)", strlen("Enter the Employee ID (e.g. 0, 1, 2)"));
        bzero(readBuffer, sizeof(readBuffer));
        readBytes = read(connFD, readBuffer, sizeof(readBuffer));
        if (readBuffer[0] < '0' || readBuffer[0] > '9')
        {
            write(connFD, "Please enter a valid integer ID!^", strlen("Please enter a valid integer ID!^"));
...
bool get_account_details(int connFD, struct Account *customerAccount)
{
    ...
    if (customerAccount == NULL)
    {
        ...
        writeBytes = write(connFD, "Enter the Account Number (e.g. 0, 1, 2)", strlen("Enter the Account Number (e.g. 0, 1, 2)"));
        if (writeBytes == -1)
        {
    ...
    if (accountNumber * (int)sizeof(struct Account) >= fileSize)
    {
        bzero(writeBuffer, sizeof(writeBuffer));
        strcpy(writeBuffer, "No account could be found for the given account number");
        strcat(writeBuffer, "^");
    ...
    sprintf(tempBuffer, "\n\tPrimary Owner ID: %d", account.owner);
    strcat(writeBuffer, tempBuffer);
    // if (account.owner != -1)
    // {
    //     sprintf(tempBuffer, "\n\tSecondary Owner ID: %d", account.owner);
    //     strcat(writeBuffer, tempBuffer);
    // }
...
bool get_customer_details(int connFD, int customerID)
{
    ...
    if (customerID == -1)
    {
        writeBytes = write(connFD, "Enter the Customer ID (e.g. 0, 1, 2)", strlen("Enter the Customer ID (e.g. 0, 1, 2)"));
        if (writeBytes == -1)
        {
